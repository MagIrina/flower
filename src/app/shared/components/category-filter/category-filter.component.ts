...
//фильтр чекбоксы, добавление в url
...
//         //если не работает, ниже альтернативный вариант верхней строчки
...
//   }
  ...
  updateFilterParam(url: string, checked: boolean) {
    const currentTypes = [...(this.activeParams.types || [])];

    let updatedTypes: string[];

    if (checked) {
      if (!currentTypes.includes(url)) {
        updatedTypes = [...currentTypes, url];
      } else {
        updatedTypes = currentTypes;
      }
    } else {
      updatedTypes = currentTypes.filter(item => item !== url);
    }
